...
import { deliveryOrderApi } from "../api/deliveryOrders";
...
export function useDeliveryOrders() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const all = await deliveryOrderApi.list();
  ...
  const create = useCallback(
    async (input) => {
      setError(null);
      try {
        await deliveryOrderApi.create(input);
        await refresh();
      } catch (e) {
  ...
  const setujui = useCallback(
    async (orderId, direkturNama, catatan) => {
      setError(null);
      try {
        await deliveryOrderApi.approve(orderId, catatan || "");
        await refresh();
      } catch (e) {
  ...
  const tolak = useCallback(
    async (orderId, direkturNama, catatan) => {
      setError(null);
      try {
        await deliveryOrderApi.reject(orderId, catatan);
        await refresh();
      } catch (e) {
  ...
  const laporkanTerkirim = useCallback(
    async ({ orderId, namaPenerimaBarang, catatanPelaporan, buktiPengirimanUrl }) => {
      setError(null);
      try {
        await deliveryOrderApi.reportDelivery({
          orderId,
          namaPenerimaBarang,
          catatanPelaporan,
          buktiPengirimanUrl,
  ...
  const submitToDirector = useCallback(
    async (orderId, docData = {}) => {
      setError(null);
      try {
        await deliveryOrderApi.submit(orderId, docData);
        await refresh();
      } catch (e) {
  ...
  const attachDocPerusahaan = useCallback(
    async (orderId, docData) => {
      setError(null);
      try {
        const res = await deliveryOrderApi.attachDocPerusahaan(orderId, docData);
        await refresh();
        return res;
      } catch (e) {
  ...
  const konfirmasiTerkirim = useCallback(
    async (orderId, catatan = "") => {
      setError(null);
      try {
        await deliveryOrderApi.confirmDelivered(orderId, catatan);
        await refresh();
      } catch (e) {
  ...
  const consolidateRun = useCallback(
    async (payload) => {
      setError(null);
      try {
        const res = await deliveryOrderApi.consolidateRun(payload);
        await refresh();
        return res;
      } catch (e) {
